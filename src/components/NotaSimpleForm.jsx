...
const STRIPE_PAYMENT_URL = "https://www.paypal.com/ncp/payment/JSS5BWLQN27N8";
const COLUMN_ALIASES = {
  nombreCompleto: ["nombre_solicitante", "nombre", "nombre_completo", "nombre_y_apellidos"],
  dni: ["dni_solicitante", "dni", "dni_nie", "documento_identidad"],
  email: ["email_solicitante", "email", "correo", "correo_electronico"],
  direccionFinca: ["direccion_finca", "direccion", "direccion_finca_registral"],
  municipio: ["municipio", "localidad"],
  provincia: ["provincia", "region"],
};

export function NotaSimpleForm() {
  const [nombreCompleto, setNombreCompleto] = useState("");
  const [dni, setDni] = useState("");
  const [email, setEmail] = useState("");
  const [direccionFinca, setDireccionFinca] = useState("");
  ...
  const handleSubmit = async (event) => {
    ...
    if (!nombreCompleto || !dni || !email || !direccionFinca || !municipio || !provincia) {
    ...
    try {
      while (attempt < maxAttempts) {
        ...
        if (nextIndex >= COLUMN_ALIASES[fieldKey].length) {
          ...
          console.warn(
            `Columna ${missingColumn} inexistente para nota simple. Omitiendo el campo ${fieldKey}.`
          );
        } else {
      ...
      if (lastError) {
        console.error("Error al insertar solicitud de nota simple:", lastError.message);
        setError("No pudimos enviar la solicitud. Intentalo de nuevo en unos minutos.");
        return;
      }
    } catch (submissionError) {
      console.error("Error inesperado al enviar la solicitud de nota simple:", submissionError);
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-md rounded bg-white p-6 shadow-md">
      <h2 className="mb-4 text-xl font-bold">Solicitud Nota Simple Registral</h2>
      ...
      <div className="mb-4">
        <label className="block text-gray-700">Direccion de la finca:</label>
        <input
          type="text"
          className="mt-1 w-full rounded border border-gray-300 p-2 shadow-sm"
          value={direccionFinca}
          onChange={(event) => setDireccionFinca(event.target.value)}
        />
      </div>
      ...
      >
        {isSubmitting ? "Enviando..." : "Enviar solicitud y pagar (14,99 EUR)"}
      </button>
    </form>
  );
}

export default NotaSimpleForm;
